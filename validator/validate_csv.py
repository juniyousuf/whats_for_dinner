...
def validate_csv_data(df):
    ...
    if(((df[df["Use By Date"] >= date_today]))['Item'].count() < 1):
        raise ValueError('Nothing in the fridge to consume')
    elif (is_measure_unit_correct is False):
        raise ValueError('Invalid item measurement unit in the file')
    elif (is_quantity_correct is False):
        raise ValueError('Invalid item quantity in the file')
    else:
        return df